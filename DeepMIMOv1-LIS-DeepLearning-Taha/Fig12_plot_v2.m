function []=Fig12_plot(My_ar,Mz_ar,M_bar,Ur_rows,Training_Size,...
    ...
    if ishandle(f12)
        ...
        for rr=numel(My_ar):-1:1
            ...
            %disp(Training_Size_valid);
            plot((Training_Size_valid*1e-3),Rate_OPTt_valid,   [Colour(rr) Marker_mat{rr, 1}],'markersize',7,'linewidth',1.2, 'DisplayName', ['Genie-Aided$,\; M \mathbin{=} ' num2str(My_ar(rr)) '\mathrm{x}' num2str(Mz_ar(rr)) '$'])
            plot((Training_Size_valid*1e-3),Rate_DLt_mat_valid,[Colour(rr) Marker_mat{rr, 2}],'markersize',7,'linewidth',1., 'DisplayName', ['$\mathrm{DL}_{\mathrm{val6400}},\; M \mathbin{=} ' num2str(My_ar(rr)) '\mathrm{x}' num2str(Mz_ar(rr)) ',\; \overline{M} \mathbin{=} 8$'])
            ...
            if epochs == 20
                ...
                plot((Training_Size_valid*1e-3),Rate_DLt_py_test_20_valid,        [Colour(rr) Marker_test{rr, 1}],          'markersize',7,'linewidth',1.0, 'DisplayName', ['$\mathrm{DL}_{\mathrm{test}      },\; M \mathbin{=} ' num2str(My_ar(rr)) '\mathrm{x}' num2str(Mz_ar(rr)) ',\; \overline{M} \mathbin{=} 8$' ])
