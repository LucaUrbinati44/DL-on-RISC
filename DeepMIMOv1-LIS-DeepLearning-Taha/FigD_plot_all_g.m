function []=FigD_plot_all(My_ar,Mz_ar,M_bar,Ur_rows,kbeams,Training_Size,Validation_Ind,Test_Ind,epochs,plot_mode, ...
...
for rr=1:1:numel(My_ar)
    ...
    if plot_mode == 2
        title(['Coverage vs Rate Threshold, RIS ', num2str(M_bar), ' active elements, pyTest model trained with ', num2str(Training_Size_dd), ' samples'],'fontsize',11)
    elseif plot_mode == 1
        title(['Coverage vs Rate Threshold, RIS ', num2str(M_bar), ' active elements, matVal model trained with ', num2str(Training_Size_dd), ' samples'],'fontsize',11)
    end
    ...
    if plot_mode == 2 || plot_mode == 1
        ...
        for b=1:size(ValTest_Ind,1) % 6200 val o 3100 test
            ...
            values_OPT_py(x,y) = MaxR_OPTt_py(rr,Training_Size_number,b);
            values_OPT_mat(x,y) = MaxR_OPTt_mat(rr,Training_Size_number,b);
        end
        ...
        plot(thresholds, punti_coperti_sul_totale_OPT, [Colour_mat{rr,1} Marker_mat{1}], 'markersize', 4, 'linewidth', 1., 'DisplayName', ['Genie_{mat}, M = ' num2str(My_ar(rr)) 'x' num2str(Mz_ar(rr))]);
        ...
        for b=1:size(ValTest_Ind,1)
            x = User_Location_norm_ValTest(1,b);
            y = User_Location_norm_ValTest(2,b);
            values_DL(x,y) = MaxR_DLt_mat(rr,Training_Size_number,b);
        end
        ...
        plot(thresholds, punti_coperti_sul_totale_DL, [Colour_mat{rr,2} Marker_mat{2}], 'markersize', 4, 'linewidth', 1., 'DisplayName', ['DL_{matVal' num2str(20) '}, M = ' num2str(My_ar(rr)) 'x' num2str(Mz_ar(rr))]);
        ...
        plot(thresholds, punti_coperti_sul_totale_OPT, [Colour_py{rr,1} Marker_py{1}], 'markersize', 4, 'linewidth', 1., 'DisplayName', ['Genie_{py}, M = ' num2str(My_ar(rr)) 'x' num2str(Mz_ar(rr))]);
        ...
        for i = 1:numel(MaxR_DLt_all)
            ...
            for b=1:size(ValTest_Ind,1)
                x = User_Location_norm_ValTest(1,b);
                y = User_Location_norm_ValTest(2,b);
                values_DL(x,y) = MaxR_DLt_all{i}(rr,Training_Size_number,b);
            end
            values_DL_plot = reshape(values_DL, numel(y_plot), []);
            punti_coperti_sul_totale_DL = arrayfun(@(th) sum(values_DL_plot(:) >= th) / numero_punti * 100, thresholds);
            ...
            %    plot(thresholds, punti_coperti_sul_totale_DL, [Colour_mat{rr,2} Marker_mat{2}], 'markersize', 4, 'linewidth', 1., 'DisplayName', ['DL_{matVal' num2str(20) '}, M = ' num2str(My_ar(rr)) 'x' num2str(Mz_ar(rr))]);
            %else
            %    plot(thresholds, punti_coperti_sul_totale_DL, [Colour_py{rr,i} Marker_py{2}], 'markersize', 4, 'linewidth', 1., 'DisplayName', ['DL_{pyTest' num2str(epochs/5*(i-1)) '}, M = ' num2str(My_ar(rr)) 'x' num2str(Mz_ar(rr))]);
            %end
            plot(thresholds, punti_coperti_sul_totale_DL, [Colour_py{rr,i+1} Marker_py{2}], 'markersize', 4, 'linewidth', 1., 'DisplayName', ['DL_{pyTest' num2str(20*i) '}, M = ' num2str(My_ar(rr)) 'x' num2str(Mz_ar(rr))]);
