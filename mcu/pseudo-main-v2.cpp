...
//Create buffer for input/output/intermediate tensors
...
void setup() {
  ...
  #ifdef MODEL_IN_RAM //Copy model from Flash to RAM
  ...
  model = tflite::GetModel(model_ram); //Load model from RAM
  #else //Load model from Flash
  model = tflite::GetModel(g_mlp_model_data);
  #endif

  //Register and access the operations needed by the model
